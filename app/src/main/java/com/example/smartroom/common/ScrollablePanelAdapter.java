...
import com.example.smartroom.bean.ClassInfo;
...
public class ScrollablePanelAdapter extends PanelAdapter {
    ...
    private List<List<ClassInfo>> classlist=new ArrayList<>();
        ...
        switch (viewType) {
            ...
            case ROOM_TYPE:
                return new RoomViewHolder(LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.listintem_room_info, parent, false));
            case ORDER_TYPE:
    ...
    private void setRoomView(int pos, RoomViewHolder viewHolder) {
     ...
     private  void setClassView(final int row, final int column, OrderViewHolder viewHolder){
        final ClassInfo classinfo=classlist.get(row-1).get(column-1);
    ...
    private void setOrderView(final int row, final int column, OrderViewHolder viewHolder) {
        ...
        if (orderInfo != null) {
            if (orderInfo.getStatus() == OrderInfo.Status.BLANK) {
                ...
                viewHolder.view.setBackgroundResource(orderInfo.isBegin() ? R.drawable.bg_room_blue_begin_with_stroke : R.drawable.bg_room_red_with_stroke);
            }
            if (orderInfo.getStatus() != OrderInfo.Status.BLANK) {
    ...
    public List<List<ClassInfo>> getClasslist() {
        return classlist;
    }

    public void setClasslist(List<List<ClassInfo>> classlist) {
